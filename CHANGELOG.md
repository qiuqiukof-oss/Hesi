...
本版本由 Hesi v0.8.1 快照衍生，作为**开源个人版 1.0.0** 独立发布：

- **定位**：单机、单用户、自带模型（BYOK）的个人 AI 协作中枢。
- **核心能力**：终端 + AI 对话 + 多 Agent 圆桌讨论（讨论→方案→实施→审核→报告）+ 一键执行流（NL → Plan → 执行）+ 63 连接器 + 知识库桥 + 定时任务 + 浏览器控制 + MCP。
- **相对企业版裁剪**：移除多用户 / RBAC / 审计 / 审批 / QQ·微信 Bot 路由 / 数字员工 / 许可校验 / 团队 / 系统监控等企业身份权限层（个人版默认回环免登录、无审计审批）。
- **保留**：`routes/bots/`（QQ/微信 Bot 代码保留，个人亦可使用）。

### 与主仓（企业版）关系
- 主仓（私有）继续承载企业版演进；本仓库为独立公开版本线（1.0.x）。
- 功能差异详见 README《关于 Hesi 企业版》对比表。
