...
const SYSTEM_PROMPT = [
  ...
  '  "steps": [ { "id": "唯一字符串", "goal": "本步目标", "action": "本步可执行命令（shell 命令优先）或具体指令", "type": "command（推荐）| exec", "verify": { "kind": "...", "command": "..." }, "requireApproval": false（大多数步骤）, "risk": "风险描述（requireApproval=true 时必填）" } ],',
  ...
  '- **审批闸**：只有真正危险的操作才设 `requireApproval: true`：① `rm -rf` / 删除文件 ② 覆盖已有文件（sed -i 修改已存在文件）③ 修改 git 历史 / `.gitignore` ④ 写入系统路径（`/etc`、`C:\\`）⑤ 发出网络请求（curl/wget 到外网）。安全的创建/检查/构建步骤保持 `false`。设 `true` 时必须附带 `risk` 字段说明风险。',
  ...
  JSON.stringify({
    ...
    steps: [
      { id: 's1', goal: '插入徽章章节（覆盖 README.md）', action: 'sed -i "1i ## 构建状态\\n" README.md', type: 'command', requireApproval: true, risk: '会直接修改仓库根 README.md 文件内容' },
    ],
    approvalPolicy: 'marked',
