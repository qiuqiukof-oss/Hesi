...
function tierMessages(rounds, opts = {}) {
  ...
  // 用 null 合并 + 钳制：原 `||` 会让配置显式置 0（如 l0Rounds:0）被默认值吞掉，
  // 且无法防御负数/NaN。
  const numOr = (v, def) => (typeof v === 'number' && Number.isFinite(v) ? v : def);
  const l0N = Math.max(0, numOr(opts.l0Rounds, DEFAULT_L0_ROUNDS));
  const l1N = Math.max(0, numOr(opts.l1Rounds, DEFAULT_L1_ROUNDS));
  const l1Max = Math.max(1, numOr(opts.l1MaxChars, DEFAULT_L1_MAX_CHARS));
  const l2Desc = Math.max(1, numOr(opts.l2DescChars, DEFAULT_L2_DESC_CHARS));
  ...
  const l1 = l1Raw.map((r, i) => {
    ...
    return {
      ...
      content: text,
      _tier: 'L1',
      _summaryOf: l1Start + i, // 原始序号（调试/回查用）：L1 段从 list[l1Start] 起
