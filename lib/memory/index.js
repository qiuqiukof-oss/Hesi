...
const MemoryStore = {
  ...
  // 多轮回滚：列出检查点栈 / 回滚到指定轮次
  listCheckpoints: (id) => session.listCheckpoints(id),
  rollbackTo: (id, seq) => session.rollbackTo(id, seq),
  // Phase 2：文件写类副作用——执行前快照 / 回滚预览
  recordSideEffect: (id, absPath, before, isNew) => session.recordSideEffect(id, absPath, before, isNew),
  getRollbackPreview: (id, seq) => session.getRollbackPreview(id, seq),
