...
function list({ q = '', limit = 0 } = {}) {
  // Exclude checkpoint shadow files (<id>.ckpt.json) stored alongside sessions
  // for rollback — they share the session id and would otherwise appear as
  // duplicate sessions in the list.
  const files = storage.listJSON(config.SESSIONS_DIR).filter((f) => !f.endsWith('.ckpt.json'));
  let sessions = files
    .map((f) => {
...
function remove(id) {
  ...
  // Drop the rollback checkpoint shadow (<id>.ckpt.json) for this session.
  // It's a single-slot snapshot of the *active* session and is regenerated on
  // the next checkpoint, so leaving it behind would just orphan it in
  // SESSIONS_DIR (and the list() filter already hides it — but it shouldn't linger).
  const ckpt = path.join(config.SESSIONS_DIR, `${id}.ckpt.json`);
  if (storage.exists(ckpt)) storage.removeFile(ckpt);
