...
// 单次生成的最大输出 token。默认 16384，覆盖复杂项目 plan 不被 max_tokens 硬截断；
// 小模型 / 低配本地 LLM 若自身上限更低，用 HESI_LLM_MAX_TOKENS 调小（如 4096）。
const LLM_BRIDGE_MAX_TOKENS = Number(process.env.HESI_LLM_MAX_TOKENS) || 16384;
...
async function _realComplete(system, user, { apiKey, provider, model, baseUrl } = {}) {
  // M2（大模型接入统一模块）：provider-config 参与 key/baseUrl 解析——
  // 请求级显式值优先，其次 provider-config（env 优先 + data/llm-providers.json，
  // 支持 deepseek/qwen/glm/kimi 等注册表 provider），最后旧 env 兜底（零回归）。
  let resolvedProvider = provider;
  let resolvedKey = apiKey;
  let resolvedBase = baseUrl;
  // bug 修复（2026-08-04）：baseUrl 缺省时也解析 provider 默认地址——
  // 否则 deepseek/qwen 等 provider 的记忆压缩/事实抽取会打到 OpenAI 官方
  if (!resolvedProvider || !resolvedKey || !resolvedBase) {
    try {
      const { resolveForChat } = require('../llm-provider/provider-client');
      const r = resolveForChat(resolvedProvider, resolvedKey, resolvedBase, 'memory');
      resolvedProvider = resolvedProvider || r.providerId;
      resolvedKey = resolvedKey || r.apiKey;
      resolvedBase = resolvedBase || r.baseUrl;
    } catch { /* 模块加载失败时回落旧逻辑 */ }
  }
  const key = resolvedKey || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || '';
  const prov = resolvedProvider || (process.env.ANTHROPIC_API_KEY ? 'anthropic' : 'openai');
  // bug 修复（2026-08-04）：本地 provider（lmstudio/ollama/vllm）无需 key——
  // key 为空时若解析出本地 provider 则允许空 key 继续（直接调本地端点）；
  // 仅「云端 provider 无 key」才报 NO_API_KEY（本地场景曾误报，导致
  // Plan 生成/讨论/记忆全部走不通 LM Studio）。
  let isCloud = true;
  try {
    const { getProviderDef, defNeedsKey } = require('../llm-provider/provider-config');
    const def = getProviderDef(prov);
    // 本地（lmstudio/ollama/vllm）与自定义 provider（用户配置端点）不强制 key
    isCloud = defNeedsKey(def);
  } catch { /* 模块缺失时保守视为云端 */ }
  if (!key && isCloud) throw new LLMError('未提供 API Key（前端未填写或存储读取失败）', 'NO_API_KEY');
  const modelName = model || (prov === 'anthropic' ? 'claude-sonnet-4-20250514' : 'gpt-4o-mini');
  try {
    // baseUrl 兜底：调用方（chat/index.js 的 extractFacts/compactIfNeeded）可能只传
    // apiKey/provider/model 漏传 baseUrl → 会打到 OpenAI 官方（离线失败）。这里统一
    // 回落 HESI_LLM_BASE_URL（本地 LLM），与 chat 主流程的本地 LLM 兜底一致（反硬编码）。
    const effBase = resolvedBase || process.env.HESI_LLM_BASE_URL || undefined;
    if (prov === 'anthropic') {
      const url = buildApiUrl(effBase, 'https://api.anthropic.com/v1', '/messages');
      const resp = await _fetchWithRetry(url, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-api-key': key, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model: modelName, max_tokens: LLM_BRIDGE_MAX_TOKENS, system, messages: [{ role: 'user', content: user }] }),
    ...
    const url = buildApiUrl(effBase, 'https://api.openai.com/v1', '/chat/completions');
    const resp = await _fetchWithRetry(url, {
      method: 'POST',
      headers: key ? { 'content-type': 'application/json', Authorization: `Bearer ${key}` } : { 'content-type': 'application/json' },
      body: JSON.stringify({
        model: modelName,
        max_tokens: LLM_BRIDGE_MAX_TOKENS,
...
module.exports = { setLLMCaller, complete, summarize, extractFacts, LLMError, LLM_BRIDGE_TIMEOUT_MS };
