...
async function compactIfNeeded(sessionId, opts = {}) {
  ...
  let summary;
  try {
    summary = await llm.summarize(oldSegText, s.summary || '', opts);
  } catch (e) {
    // LLM 不可用（缺 Key / 网络 / 超时）→ 不压缩、保留原文，避免崩溃对话流。
    // routes/chat/index.js:355 在无 .catch 情况下 await 本函数，必须降级而非抛出。
    return { degraded: true, reason: 'llm-unavailable', error: e && e.message };
  }
  if (!summary) {
