...
//   CONTINUE —— 继续修订重跑
//
// （STALLED 软收敛暂缓：现有判定顺序下被 STALL/DRIFT 覆盖不可达；
//   P2 引入 Reviewer 时再按「零 Delta 软收敛 → 触发质量审查」语义回归。）
//
// 纯函数纪律：decide() 只读传入 round，不碰执行、不碰 IO；
// 状态仅保存在实例字段（sigs 窗口 / lastSig / lastPlanHash）。
// ============================================================
...
class ReplanController {
   ...
   */
  constructor({ accHash = null, budgetFrozen = null, windowSize = WINDOW_DEFAULT } = {}) {
    ...
    this.lastSig = null;        // 完整状态签名（STALL：相邻同签名用）
    this.lastPlanHash = null;   // plan hash（DRIFT：计划漂移比对用）
  }
   ...
   * @returns {{ v: 'DONE'|'STALL'|'OSCILL'|'DRIFT'|'ESCALATE'|'CONTINUE', why?: string, partial?: boolean }}
   */
  decide(round = {}) {
    ...
    if (this.lastPlanHash !== null && round.planHash !== null && round.planHash !== undefined
      && round.planHash !== this.lastPlanHash && !round.gitDiff) {
    ...
    this.lastSig = sig;
    this.lastPlanHash = round.planHash || this.lastPlanHash;

    return { v: 'CONTINUE' };
  }
}

module.exports = { ReplanController, normBudget, WINDOW_DEFAULT };
