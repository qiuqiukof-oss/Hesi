...
/**
 * 运行时解析 Hesi 服务 URL —— 端口从 process.env.PORT 动态读取（兜底 4264），
 * 与 lib/ensure-agent-config.js / server.js 的端口解析口径一致。
 * 不在模块顶层快照，确保 PORT 变更后每次调用都取到最新值（规避 require 缓存）。
 * @returns {string} 例如 "http://127.0.0.1:4264"
 */
function resolveHesiUrl() {
...
module.exports = { HESI_RUNTIME, HESI_CORE_CAPABILITIES, resolveHesiUrl };
