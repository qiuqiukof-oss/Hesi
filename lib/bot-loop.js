          ...
          if (t === 'GROUP_AT_MESSAGE_CREATE' || t === 'C2C_MESSAGE_CREATE') {
            ...
            handleInbound('qq', inbound, {
              reply: async (cid, text) => {
                ...
                await sendMessage(cid, text, { chatType: t === 'GROUP_AT_MESSAGE_CREATE' ? 2 : 1 });
              },
            });
          }
        } else if (msg.op === 7) {
...
function startWechatLoop() {
  ...
  const tick = async () => {
    if (stopped) return;
    const { getUpdates, sendMessage } = require('../routes/bots/wechat-bot');
    const r = await getUpdates(buf);
    if (stopped) return;
    if (r.expired) {
      console.warn('[bot-loop] 微信会话过期，请重新扫码');
      return; // 保留循环但不再有消息（等重新扫码后由配置变更重启）
    }
    if (r.ok) {
      retryDelay = 3000;
      buf = r.buf || buf;
      for (const m of r.msgs || []) {
        const content = m && (m.text || m.content || '');
        ...
        handleInbound('wechat-bot', inbound, {
          reply: async (cid, text) => {
            ...
            await sendMessage(contextToken, text);
          },
        });
      }
    } else {
    ...
    if (!stopped) timer = setTimeout(tick, r.ok ? 0 : retryDelay);
  };
...
module.exports = { startAll, stopAll, startLoops, handleInbound };
