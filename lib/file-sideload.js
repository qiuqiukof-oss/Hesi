...
//
// 背景：read_file 结果超过侧载阈值（保守 ~10K 字符）原先走通用
// ToolResultTruncator（截到 ~4000 token），被截掉的内容 AI 永远看不到、也不知道文件全貌。
...
/** 触发侧载的字符阈值（保守 ~10K，使 18KB 文件也能侧载，避免整文件进上下文） */
const SIDELOAD_THRESHOLD = 10240;
/** 头部保留行数 */
