...
const config = require('../config'); // isEnterprise：个人版恒本地模式，企业版看 .env.local
...
function isLocalDshMode() {
  // 个人版（单机单用户）：DSH Web UI 不受限，恒本地模式（无需 .env.local）
  // 企业版：仅机主本地模式（.env.local 存在）可用，否则强制走 Phase 2 引擎
  try {
    if (!config.isEnterprise) return true;
    return fs.existsSync(path.join(DSH_ROOT_DIR, '.env.local'));
  } catch { return false; }
}
