...
import dshInit from '../dsh-init.js'
...
/**
 * 解析实际使用的 composition 文件路径。
 * 企业模式：剔除 permission-presets 中的 danger-full-access（使其不可选），
 * 写一份 composition.active.yml 供 boot 使用；非企业模式：原样使用。
 */
async function resolveCompositionPath() {
  const base = fileURLToPath(COMPOSITION_URL)
  // 个人版 config.js 无 getAuthMode()，用等价的 isEnterprise（AUTH_MODE === 'enterprise'）
  if (!config.isEnterprise) return base
  try {
    ...
    const active = path.join(path.dirname(base), 'composition.active.yml')
    fs.writeFileSync(active, stripped, 'utf-8')
    return active
  } catch (e) {
    console.warn('[dsh2] 生成企业模式 composition 失败，回退默认：', e && e.message)
    return base
  }
}
...
function getDshProvider() {
  // HESI_DSH_PROVIDER env가 있으면 우선 사용；없으면 Hesi「模型服务」페이지의
  // 사용자 기본 선택 provider（data/llm-providers.json 的 _default）를 읽어온다.
  // "qq" (Agnes) 같은 커스텀 provider도 정상적으로 선택되도록 함.
  return process.env.HESI_DSH_PROVIDER || providerConfig.getDefaultProvider() || 'deepseek'
}
...
function applyHesiEnv() {
  // DSH_HOME 锚定到 Hesi 项目目录，不依赖 C 盘
  dshInit.init()
  ...
  // DSH 沙箱/审批模式跟随 Hesi 鉴权模式：
  //   enterprise → workspace-write（受限、Full access 不可选）
  //   非 enterprise(local/机主) → danger-full-access（完全不受限，等同当前状态）
  process.env.DSH_PERMISSION_MODE =
    config.isEnterprise ? 'workspace-write' : 'danger-full-access'
  try {
...
export async function ensureEngine() {
  ...
  if (!bootPromise) {
    bootPromise = (async () => {
      applyHesiEnv()
      ctx = await boot('hesi-dsh2', await resolveCompositionPath())
      return ctx
    })()
  }
  return bootPromise
...
/**
 * 会话缓存 key：userId + hesiSessionId 双因子，防止不同用户用相同
 * sessionId 时误拿到对方的 agent（否则 A 建的 agent 会带着 A 的 cwd
 * 被 B 复用 —— 用户隔离被击穿）。未登录内部调用降级为纯 sessionId。
 */
function agentKey(userId, hesiSessionId) {
  return userId && userId !== 'local' && userId !== 'system'
    ? `${userId}::${hesiSessionId}`
    : hesiSessionId
}

/** 取（或创建）某 Hesi 会话对应的 DSH Agent。@returns {Promise<object>} */
export async function getOrCreateAgent(hesiSessionId, { cwd, userId } = {}) {
  const engine = await ensureEngine()
  const key = agentKey(userId, hesiSessionId)
  if (agentByHesi.has(key)) return agentByHesi.get(key)
  ...
  const { agent } = await agents.create({
    sessionId: SessionId(`hesi-${key}-${PROC_TAG}`),
    meta: { cwd: cwd || workspace.getWorkspace() },
    agentOptions: { provider: selection.provider, model: selection.model },
  })
  agentByHesi.set(key, agent)
  return agent
}

/** 销毁某 Hesi 会话的 DSH Agent（开新会话用）。@returns {Promise<boolean>} */
export async function resetSession(hesiSessionId, { userId } = {}) {
  const key = agentKey(userId, hesiSessionId)
  const agent = agentByHesi.get(key)
  if (!agent) return false
  try { await agent.session.dispose() } catch { /* ignore */ }
  agentByHesi.delete(key)
  return true
}
 ...
 */
export async function sendMessage(hesiSessionId, text, { cwd, userId, onEvent } = {}) {
  const engine = await ensureEngine()
  const agent = await getOrCreateAgent(hesiSessionId, { cwd, userId })
  const sessions = engine.get('sessions')
