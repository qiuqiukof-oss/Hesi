...
function maybeGitSnapshot(file, partial, projectId) {
  ...
  try {
    ...
    execFileSync('git', ['-C', root, 'add', file], { stdio: 'ignore' });
    const summary = deriveSummary(partial);
    execFileSync('git', ['-C', root, 'commit', '-m', `blackboard(${projectId}): ${summary}`], { stdio: 'ignore' });
  } catch {
    /* git 不可用 / 文件被 ignore / 无变更：静默降级 */
  }
}
 ...
 *   - expectedVersion：若提供且 ≠ 当前 version → 抛 BlackboardConflictError
 *   - expectedChecksums：{ path: hash }，若任一 path 当前 hash 不符 → 抛 BlackboardConflictError
 * @returns {object} 更新后的完整状态
 * @throws {BlackboardConflictError} 乐观锁/checksum 冲突（显式报错，绝不静默覆盖）
 */
...
/** 全量覆写（初始化/重置用）。经 withLock 保证同文件并发安全。 */
