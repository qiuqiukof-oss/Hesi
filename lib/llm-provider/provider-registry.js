...
const REGISTRY = [
  ...
  },
  {
    ...
    defaultBaseUrl: 'http://localhost:11434/v1', models: [],
  },
  {
    id: 'lmstudio', name: 'LM Studio(本地)', kind: 'local', apiType: 'openai-compat',
    // 球总要求：base URL 不含 /v1（服务根地址）；buildApiUrl 会自动补 /v1
    defaultBaseUrl: 'http://localhost:1234', models: [],
  },
  {
    id: 'vllm', name: 'vLLM(本地)', kind: 'local', apiType: 'openai-compat',
    defaultBaseUrl: 'http://localhost:8000/v1', models: [],
  },
];
...
// bug 修复（2026-08-04）：不再导出可变 REGISTRY 本体——外部统一走 getRegistry()
// （返回副本）/ getProvider()，防止运行时被外部修改注册表
module.exports = { getRegistry, getProvider };
