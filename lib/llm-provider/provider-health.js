...
const { getConfig, getProviderDef, getAllDefs } = require('./provider-config');
...
async function checkHealth(providerId, opts = {}) {
  ...
  // 内置云端无 key → unconfigured（不探测，省调用）；本地/自定义（用户配置端点）
  // 无 key 也继续探测（内网中转/自建网关常见无 key）
  if (def.kind === 'cloud' && !def._custom && !cfg.apiKey) {
  ...
  try {
    ...
    // bug 修复（2026-08-04）：anthropic 鉴权用 x-api-key（Bearer 不被接受，
    // 此前 anthropic 健康探测恒 degraded）；openai-compat 用 Bearer
...
async function healthAll(opts = {}) {
  ...
  return results.map((r) => {
    const def = getProviderDef(r.providerId);
...
function resolveWithFallback(preferredId, opts = {}) {
  ...
  if (preferredId) {
    const def = getProviderDef(preferredId);
    if (def) {
