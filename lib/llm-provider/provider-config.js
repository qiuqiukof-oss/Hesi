...
/** 从 env 读取某 provider 的 model 覆盖（HESI_LLM_<ID>_MODEL）。@param {string} id @returns {string} */
function readModelFromEnv(id) {
  const key = `HESI_LLM_${id.toUpperCase()}_MODEL`;
  return process.env[key] || '';
}
...
function getConfig(id) {
  ...
  const envModel = readModelFromEnv(id);

  const apiKey = envKey || fileCfg.apiKey || def._customApiKey || '';
  const baseUrl = envBase || fileCfg.baseUrl || def.defaultBaseUrl;
  // env 模型覆盖（HESI_LLM_<ID>_MODEL）优先，与 baseUrl 的 env 覆盖对称（2026-08-15）
  const model = envModel || fileCfg.model || def._customModel || (def.models && def.models[0]) || '';

  const hasEnv = !!(envKey || envBase || envModel);
