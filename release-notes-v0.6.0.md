...
本版本按「分阶段小发布」策略，先交付清理与稳定性底座；全自动 Phase 1（auto-Planner / 反思重规划环 / RAG 回流 / 运行时逐工具拦截）、圆桌画廊 UI、讨论衔接将在 **v0.6.1 – v0.6.3** 陆续发布。
...
- 版本号 `0.5.8 → 0.6.0`。
- 严格按「禁 `git add -A`」逐文件提交，三笔逻辑清晰的 commit（B4 / D4 / D3+bump）。

## ✅ 质量门

- `node scripts/test.mjs`：**139** 通过 / 0 失败（含新增 `normalizeTranscript` 用例）。
- `eslint`：**0 error**（207 warning，与历史基线持平）。
- `npm run build:lazy` 通过。
